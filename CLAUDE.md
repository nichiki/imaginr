...
└── lib/
    ...
    ├── ollama-api.ts         # Ollamaクライアント（LLMエンハンサー）
...
- **ギャラリー**:
  - サムネイル一覧（6列グリッド）
  - ページネーション（無限スクロール、50件ずつ読み込み）
  - ソート切り替え（新しい順/古い順）
  - 一括選択モード（Shift+クリックで範囲選択対応）
  - 一括削除
  - 全文検索（FTS5）
  - 拡大表示（ビューポート90%サイズ）
  - キーボードナビゲーション（←→で移動、Escで閉じる）
  - 画像ダウンロード機能（ファイル保存ダイアログ）
  - 生成パラメータ表示（seed, negative prompt, overrides）
...
**完了**
- [x] Mac版のビルド対応
- [x] 辞書編集機能（UI上での追加・編集・削除）
- [x] LLMエンハンサー（Ollama連携、プリセット管理）
- [x] ギャラリー機能（ページネーション、ソート、一括選択・削除、全文検索）
- [x] 生成パラメータ保存・表示（seed, negative prompt, overrides）
- [x] アプリ名決定 + リポジトリ名変更・public化
- [x] アイコンデザイン
- [x] マニュアル・ドキュメント作成
- [x] CI/CD（GitHub Actionsでの自動ビルド・リリース）
- [x] 複数タブ対応
- [x] エディタ左右分割対応
- [x] レイアウトモード切り替え（上段のみ/下段のみ/全表示）

**残タスク**
- [x] 辞書・テンプレート・スニペットのプリセット整備
- [x] デバッグ・動作確認
- [x] 英語対応
...
### 機能
- **プリセット管理**: システムプロンプトを名前付きで複数登録・切り替え
- **エンハンス実行**: YAMLプロンプトをLLMで自然言語に変換
- **キャッシュ機構**: 同一YAML+プリセットの組み合わせは再利用
- **生成時自動エンハンス**: 「Enhance before generate」オプションで生成前に自動実行

### フロー
```
YAML → (Ollama LLM) → 自然言語プロンプト → ComfyUI → 画像生成
```

### 設定
- Ollamaエンドポイント（デフォルト: http://localhost:11434）
- 使用モデル選択（Ollamaにインストール済みモデルから選択）
- Temperature設定
- エンハンサープリセット（システムプロンプト）の管理
